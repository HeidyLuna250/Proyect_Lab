...
const Ventas = () => {
  ...
  useEffect(() => {
    if (ventaAEditar) {
      const cliente = clientes.find(
        (c) => c.id_cliente === ventaAEditar.id_cliente,
      );
      const empleado = empleados.find(
        (e) => e.id_empleado === ventaAEditar.id_empleado,
      );
  ...
  useEffect(() => {
    const total = detalles.reduce(
      (sum, det) => sum + det.cantidad * det.precio,
      0,
    );
  ...
  useEffect(() => {
    if (!textoBusqueda.trim()) {
      ...
      const filtradas = ventas.filter(
        (v) =>
          `${v.clientes?.nombre_cliente || ""} ${v.clientes?.apellido_cliente || ""}`
            .toLowerCase()
            .includes(textoLower) ||
